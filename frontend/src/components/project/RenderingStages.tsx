...
/**
 * The Rendering Engine pipeline progress - REAL, per-stage status from the
 * backend, grouped into the engine's sections (Inputs, Build Plan, Apply Edits,
 * Render & Publish).
 *
 * Each row reflects a stage's genuine state: plan/validation stages complete for
 * real; execution stages that need a renderer (e.g. FFmpeg) report honestly as
...
import { STAGE_GROUPS, stageTally } from "@/lib/rendering";
import { useRerunRenderStage } from "@/lib/queries";
...
> = {
  ...
  running: { label: "Rendering", tone: "text-accent", icon: "running" },
...
function StageIcon({ status }: { status: RenderStageStatus }) {
...
function StageRow({ stage, projectId }: { stage: RenderStage; projectId: string }) {
  const [open, setOpen] = useState(false);
  const rerun = useRerunRenderStage(projectId);
    ...
    <li className="flex gap-3 py-2">
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-start justify-between gap-3">
          ...
          >
            <span className={`truncate text-sm ${active ? "font-medium text-white" : "text-muted"}`}>
              {stage.label}
            </span>
            {detail && (
              <ChevronDownIcon
...
export function RenderingStages({
  render,
  isLoading,
}: {
  ...
  if (!render) {
    return (
      <div className="flex items-center gap-3 text-sm text-muted">
        ...
        Rendering executes the edit on a finished timeline. Start it with “Run render”.
      </div>
    );
  }

  const byName = new Map(render.stages.map((s) => [s.stage, s]));
  const isRunning = render.status === "running" || render.status === "pending";
  const tally = stageTally(render.stages);
    ...
    <div>
      <div className="mb-5 flex flex-wrap items-center justify-between gap-2">
        <p className="text-sm text-muted">
          ...
          {tally.unavailable > 0 && <span className="text-amber-300"> · {tally.unavailable} unavailable</span>}
          {tally.failed > 0 && <span className="text-red-300"> · {tally.failed} failed</span>} of{" "}
          {tally.total} stages
          {isRunning && <span className="ml-2 text-accent">· rendering…</span>}
        </p>
        <span className="text-[11px] uppercase tracking-wide text-muted">
          Render pipeline v{render.pipeline_version}
        </span>
      </div>
      ...
      <div className="space-y-5">
        {STAGE_GROUPS.map((group) => {
          const stages = group.stages
            ...
            .filter((s): s is RenderStage => s != null);
          ...
          return (
            <div key={group.title}>
              ...
              <ul className="divide-y divide-white/5">
                {stages.map((stage) => (
                  <StageRow key={stage.stage} stage={stage} projectId={render.project_id} />
                ))}
              </ul>
            </div>
          );
