...
/**
 * The Story Engine pipeline progress — REAL, per-stage narrative-understanding
 * status reported by the backend.
 *
 * Each row reflects a story stage's genuine state: completed stages are marked
 * done; stages that lack the inputs they need (most need a transcript) are shown
...
function StageIcon({ status }: { status: StoryStageStatus }) {
...
function StageRow({
  ...
  const rerun = useRerunStoryStage(projectId);
  ...
  if (!story) {
    return (
      <div className="flex items-center gap-3 text-sm text-muted">
        ...
        Story analysis begins automatically once the video understanding completes…
      </div>
    );
  }

  const isRunning = story.status === "running" || story.status === "pending";

  return (
    <div>
      <div className="mb-5 flex flex-wrap items-center justify-between gap-2">
        <p className="text-sm text-muted">
          <span className="font-medium text-white">{story.completed_stages}</span> of{" "}
          {story.total_stages} story signals understood
          {isRunning && <span className="ml-2 text-accent">· analyzing…</span>}
        </p>
        <span className="text-[11px] uppercase tracking-wide text-muted">
          Story pipeline v{story.pipeline_version}
        </span>
      </div>
