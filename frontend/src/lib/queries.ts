...
export const queryKeys = {
  ...
  bobaEditorialDecisions: (id: string) =>
    ["boba", "projects", id, "editorial-decisions"] as const,
  bobaExplanations: (id: string) => ["boba", "projects", id, "explanations"] as const,
  bobaCreativeDirectionV2: (id: string) =>
...
export function useBobaRootCauseAnalyzer(projectId: string) {
  return useQuery<BobaRootCauseAnalyzerSetV1 | null>({
    queryKey: queryKeys.bobaRootCauseAnalyzer(projectId),
    queryFn: async () => {
      try {
        return await api.getBobaRootCauseAnalyzer(projectId);
      } catch (error) {
        if (error instanceof ApiClientError && error.status === 404) return null;
        throw error;
      }
...
export function useGenerateBobaRootCauseAnalyzer(projectId: string) {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (input: BobaRootCauseAnalyzerGenerateInputV1 = {}) =>
      api.generateBobaRootCauseAnalyzer(projectId, input),
    onSuccess: (result, input) => {
      if (!input?.dry_run) {
        queryClient.setQueryData(
...
export function useExportBobaRootCauseAnalyzer(projectId: string) {
  return useMutation({
    mutationFn: () => api.exportBobaRootCauseAnalyzer(projectId),
  });
}

export function useResetBobaRootCauseAnalyzer(projectId: string) {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: () => api.resetBobaRootCauseAnalyzer(projectId),
    onSuccess: () => {
      queryClient.setQueryData(
        queryKeys.bobaRootCauseAnalyzer(projectId),
        null,
...
export function useBobaCodeSurgeon(projectId: string) {
  return useQuery<BobaCodeSurgeonSetV1 | null>({
    queryKey: queryKeys.bobaCodeSurgeon(projectId),
    queryFn: async () => {
      try {
        return await api.getBobaCodeSurgeon(projectId);
      } catch (error) {
        if (error instanceof ApiClientError && error.status === 404) return null;
        throw error;
      }
...
export function useProposeBobaCodeSurgeon(projectId: string) {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (input: BobaCodeSurgeonProposalInputV1) =>
      api.proposeBobaCodeSurgeon(projectId, input),
    onSuccess: (result) => {
      queryClient.setQueryData(queryKeys.bobaCodeSurgeon(projectId), result);
    },
  });
}

export function useValidateBobaCodeSurgeonPatch(projectId: string) {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (input: BobaCodeSurgeonProposalInputV1) =>
      api.validateBobaCodeSurgeonPatch(projectId, input),
    onSuccess: (result) => {
      queryClient.setQueryData(queryKeys.bobaCodeSurgeon(projectId), result);
    },
  });
...
export function useExecuteBobaCodeSurgeonPatch(projectId: string) {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (input: BobaCodeSurgeonExecuteInputV1) =>
      api.executeApprovedBobaCodeSurgeonPatch(projectId, input),
    onSuccess: (result) => {
      queryClient.setQueryData(queryKeys.bobaCodeSurgeon(projectId), result);
    },
  });
...
export function usePrepareBobaCodeSurgeonCommit(projectId: string) {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (input: BobaCodeSurgeonCommitInputV1) =>
      api.prepareBobaCodeSurgeonLocalCommit(projectId, input),
    onSuccess: (result) => {
      queryClient.setQueryData(queryKeys.bobaCodeSurgeon(projectId), result);
    },
  });
...
export function useExportBobaCodeSurgeon(projectId: string) {
  return useMutation({
    mutationFn: () => api.exportBobaCodeSurgeon(projectId),
  });
}

export function useResetBobaCodeSurgeon(projectId: string) {
  const queryClient = useQueryClient();
  return useMutation({
...
export function useBobaCandidateClipDiscovery(projectId: string) {
  return useQuery<BobaCandidateClipDiscoveryV1 | null>({
    queryKey: queryKeys.bobaCandidateClipDiscovery(projectId),
    queryFn: async () => {
      try {
        return await api.getBobaCandidateClipDiscovery(projectId);
      } catch (error) {
        if (error instanceof ApiClientError && error.status === 404) return null;
        throw error;
      }
...
export function useDiscoverBobaCandidateClips(projectId: string) {
  const qc = useQueryClient();
  return useMutation({
...
export function useBobaEditorialDecisions(projectId: string) {
  return useQuery<BobaEditorialDecisionSetV1 | null>({
    queryKey: queryKeys.bobaEditorialDecisions(projectId),
    queryFn: async () => {
      try {
        return await api.getBobaEditorialDecisions(projectId);
      } catch (error) {
        if (error instanceof ApiClientError && error.status === 404) return null;
        throw error;
      }
...
export function useCreateBobaEditorialDecisions(projectId: string) {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: () => api.createBobaEditorialDecisions(projectId),
    onSuccess: (result) => {
      qc.setQueryData(queryKeys.bobaEditorialDecisions(projectId), result);
    },
  });
}

export function useBobaExplanations(projectId: string) {
  return useQuery<BobaExplanationSetV1 | null>({
    queryKey: queryKeys.bobaExplanations(projectId),
    queryFn: async () => {
      try {
        return await api.getBobaExplanations(projectId);
      } catch (error) {
        if (error instanceof ApiClientError && error.status === 404) return null;
        throw error;
      }
...
export function useCreateBobaExplanations(projectId: string) {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: () => api.createBobaExplanations(projectId),
    onSuccess: (result) => {
      qc.setQueryData(queryKeys.bobaExplanations(projectId), result);
    },
  });
}

export function useBobaCreativeDirectionV2(projectId: string) {
  return useQuery<BobaCreativeDirectionSetV2 | null>({
    queryKey: queryKeys.bobaCreativeDirectionV2(projectId),
    queryFn: async () => {
      try {
        return await api.getBobaCreativeDirectionV2(projectId);
      } catch (error) {
        if (error instanceof ApiClientError && error.status === 404) return null;
        throw error;
      }
...
export function useCreateBobaCreativeDirectionV2(projectId: string) {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: () => api.createBobaCreativeDirectionV2(projectId),
    onSuccess: (result) => {
      qc.setQueryData(queryKeys.bobaCreativeDirectionV2(projectId), result);
    },
  });
}

export function useBobaClipBriefs(projectId: string) {
  return useQuery<BobaClipBriefSetV1 | null>({
    queryKey: queryKeys.bobaClipBriefs(projectId),
    queryFn: async () => {
      try {
        return await api.getBobaClipBriefs(projectId);
      } catch (error) {
        if (error instanceof ApiClientError && error.status === 404) return null;
        throw error;
      }
...
export function useCreateBobaClipBriefs(projectId: string) {
  const qc = useQueryClient();
  return useMutation({
...
export function useBobaExperimentation(projectId: string) {
  return useQuery<BobaExperimentationSetV1 | null>({
    queryKey: queryKeys.bobaExperimentation(projectId),
    queryFn: async () => {
      try {
        return await api.getBobaExperimentation(projectId);
      } catch (error) {
        if (error instanceof ApiClientError && error.status === 404) return null;
        throw error;
      }
...
export function useGenerateBobaExperimentation(projectId: string) {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (input: { creator_id?: string; dry_run?: boolean } = {}) =>
      api.generateBobaExperimentation(projectId, input),
    onSuccess: (result, input) => {
      if (!input?.dry_run) {
        queryClient.setQueryData(queryKeys.bobaExperimentation(projectId), result);
      }
    },
  });
}

export function useExportBobaExperimentation(projectId: string) {
  return useMutation({
    mutationFn: () => api.exportBobaExperimentation(projectId),
  });
}

export function useResetBobaExperimentation(projectId: string) {
  const queryClient = useQueryClient();
  return useMutation({
