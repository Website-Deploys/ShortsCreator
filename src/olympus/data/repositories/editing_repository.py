"""Storage-backed editing repository.

Persists a project's edit timelines through the storage abstraction, mirroring
the other engines' repository layout so all five behave identically with respect
to durability and rerunnability:

- An **index** at ``editing/{project_id}/index.json`` (overall status +
  per-stage summaries, no heavy data).
- One **stage artifact** per stage at
  ``editing/{project_id}/stages/{stage}.json`` holding that stage's full result
  (its per-clip events and, for validation, the assembled timelines).

Loading reassembles the complete :class:`EditingAnalysis` from the index plus
each stage artifact. A database-backed implementation can later replace this
behind the same :class:`EditingRepository` contract.
"""
...
class StorageEditingRepository(EditingRepository):
    """Persist editing analyses as JSON documents in object storage."""
    ...
    async def load(self, project_id: str) -> EditingAnalysis | None:
        ...
        try:
            ...
            raise StorageError(
                "Stored editing index is corrupt.", details={"project_id": project_id}
            ) from exc

        stages: list[EditingStageResult] = []
        for summary in raw.get("stages", []):
            stage_name = summary["stage"]
            full = await self._load_stage(project_id, stage_name)
            stages.append(full if full is not None else EditingStageResult.from_dict(summary))

        return EditingAnalysis(
            project_id=raw["project_id"],
            pipeline_version=str(raw.get("pipeline_version", "1")),
            status=EditingStatus(raw.get("status", "pending")),
            created_at=datetime.fromisoformat(raw["created_at"]),
            updated_at=datetime.fromisoformat(raw["updated_at"]),
            stages=stages,
        )

    async def _load_stage(self, project_id: str, stage: str) -> EditingStageResult | None:
        key = _stage_key(project_id, stage)
        if not await self._storage.exists(key):
            return None
        try:
            return EditingStageResult.from_dict(json.loads(await self._storage.get(key)))
        except (json.JSONDecodeError, KeyError, ValueError):
            log.warning("skipping_corrupt_editing_stage", project_id=project_id, stage=stage)
            return None
