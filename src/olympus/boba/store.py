...
class BobaMemoryStore:
    ...
    @staticmethod
    def _validate_error_doctor_review_record_id(value: str, *, label: str) -> str:
        if not re.fullmatch(r"[A-Za-z0-9_.:-]{1,180}", value):
            raise ValidationError(f"Invalid BOBA error doctor review {label}.")
        return value

    def boba_error_doctor_review_path(self, project_id: str) -> Path:
        return self._path(project_id, "error_doctor_review/index.json")

    def boba_error_doctor_review_registry_path(
        self, project_id: str, record_id: str
    ) -> Path:
        self._validate_error_doctor_review_record_id(record_id, label="registry id")
        return self._path(project_id, f"error_doctor_review/registries/{record_id}.json")

    def boba_error_doctor_review_session_path(self, project_id: str, record_id: str) -> Path:
        self._validate_error_doctor_review_record_id(record_id, label="session id")
        return self._path(project_id, f"error_doctor_review/sessions/{record_id}.json")

    def boba_error_doctor_review_snapshot_path(self, project_id: str, record_id: str) -> Path:
        self._validate_error_doctor_review_record_id(record_id, label="snapshot id")
        return self._path(project_id, f"error_doctor_review/snapshots/{record_id}.json")

    def boba_error_doctor_review_action_path(self, project_id: str, record_id: str) -> Path:
        self._validate_error_doctor_review_record_id(record_id, label="action id")
        return self._path(project_id, f"error_doctor_review/actions/{record_id}.json")

    def boba_error_doctor_review_receipt_path(self, project_id: str, record_id: str) -> Path:
        self._validate_error_doctor_review_record_id(record_id, label="receipt id")
        return self._path(project_id, f"error_doctor_review/receipts/{record_id}.json")

    def boba_error_doctor_review_event_cursor_path(
        self, project_id: str, session_id: str
    ) -> Path:
        self._validate_error_doctor_review_record_id(session_id, label="session id")
        return self._path(
            project_id, f"error_doctor_review/event_cursors/{session_id}.json"
        )

    def _write_boba_error_doctor_review_record(
        self, path: Path, payload: dict[str, Any], *, scope: str
    ) -> None:
        safe = sanitize_memory_payload(
            dict(payload),
            max_excerpt_chars=max(self.max_excerpt_chars, 1_200),
            path=f"boba.error_doctor_review.{scope}",
        )
        self._atomic_write_compact(path, safe)

    def _write_immutable_boba_error_doctor_review_record(
        ...
        with self._lock:
            if path.exists():
                ...
                if existing != safe:
                    raise ValidationError(
                        f"BOBA error doctor review {label} are immutable."
                    )
                return
            self._atomic_write_compact(path, safe)

    def save_boba_error_doctor_review(
        self, project_id: str, payload: dict[str, Any]
    ) -> None:
        self._write_boba_error_doctor_review_record(
            self.boba_error_doctor_review_path(project_id), payload, scope="index"
        )

    def load_boba_error_doctor_review(self, project_id: str) -> dict[str, Any] | None:
        raw = self._read(self.boba_error_doctor_review_path(project_id), None)
        return raw if isinstance(raw, dict) else None

    def save_boba_error_doctor_review_registry(
        self, project_id: str, record_id: str, payload: dict[str, Any]
    ) -> None:
        self._write_immutable_boba_error_doctor_review_record(
            self.boba_error_doctor_review_registry_path(project_id, record_id),
            payload,
            scope="registry",
            label="registry snapshots",
        )

    def load_boba_error_doctor_review_registry(
        self, project_id: str, record_id: str
    ) -> dict[str, Any] | None:
        raw = self._read(
            self.boba_error_doctor_review_registry_path(project_id, record_id), None
        )
        return raw if isinstance(raw, dict) else None

    def save_boba_error_doctor_review_session(
        self, project_id: str, record_id: str, payload: dict[str, Any]
    ) -> None:
        self._write_boba_error_doctor_review_record(
            self.boba_error_doctor_review_session_path(project_id, record_id),
            payload,
            scope="session",
        )

    def load_boba_error_doctor_review_session(
        self, project_id: str, record_id: str
    ) -> dict[str, Any] | None:
        raw = self._read(
            self.boba_error_doctor_review_session_path(project_id, record_id), None
        )
        return raw if isinstance(raw, dict) else None

    def save_boba_error_doctor_review_snapshot(
        self, project_id: str, record_id: str, payload: dict[str, Any]
    ) -> None:
        self._write_boba_error_doctor_review_record(
            self.boba_error_doctor_review_snapshot_path(project_id, record_id),
            payload,
            scope="snapshot",
        )

    def load_boba_error_doctor_review_snapshot(
        self, project_id: str, record_id: str
    ) -> dict[str, Any] | None:
        raw = self._read(
            self.boba_error_doctor_review_snapshot_path(project_id, record_id), None
        )
        return raw if isinstance(raw, dict) else None

    def save_boba_error_doctor_review_action(
        self, project_id: str, record_id: str, payload: dict[str, Any]
    ) -> None:
        self._write_immutable_boba_error_doctor_review_record(
            self.boba_error_doctor_review_action_path(project_id, record_id),
            payload,
            scope="action",
            label="action requests",
        )

    def load_boba_error_doctor_review_action(
        self, project_id: str, record_id: str
    ) -> dict[str, Any] | None:
        raw = self._read(
            self.boba_error_doctor_review_action_path(project_id, record_id), None
        )
        return raw if isinstance(raw, dict) else None

    def save_boba_error_doctor_review_receipt(
        self, project_id: str, record_id: str, payload: dict[str, Any]
    ) -> None:
        self._write_immutable_boba_error_doctor_review_record(
            self.boba_error_doctor_review_receipt_path(project_id, record_id),
            payload,
            scope="receipt",
            label="receipts",
        )

    def load_boba_error_doctor_review_receipt(
        self, project_id: str, record_id: str
    ) -> dict[str, Any] | None:
        raw = self._read(
            self.boba_error_doctor_review_receipt_path(project_id, record_id), None
        )
        return raw if isinstance(raw, dict) else None

    def load_boba_error_doctor_review_receipt_for_action(
        self, project_id: str, action_id: str
    ) -> dict[str, Any] | None:
        self._validate_error_doctor_review_record_id(action_id, label="action id")
        directory = self._path(project_id, "error_doctor_review/receipts")
        if not directory.exists():
            return None
        for path in sorted(directory.glob("*.json"))[:256]:
            raw = self._read(path, None)
            if isinstance(raw, dict) and (
                raw.get("error_doctor_action_request_id") == action_id
            ):
                return raw
        return None

    def delete_boba_error_doctor_review_session(
        self, project_id: str, session_id: str
    ) -> bool:
        path = self.boba_error_doctor_review_session_path(project_id, session_id)
        with self._lock:
            existed = path.exists()
            path.unlink(missing_ok=True)
            self.boba_error_doctor_review_event_cursor_path(
                project_id, session_id
            ).unlink(missing_ok=True)
        return existed

    def save_boba_error_doctor_review_event_cursor(
        self, project_id: str, session_id: str, payload: dict[str, Any]
    ) -> None:
        self._write_boba_error_doctor_review_record(
            self.boba_error_doctor_review_event_cursor_path(project_id, session_id),
            payload,
            scope="event_cursor",
        )

    def reset_boba_error_doctor_review_metadata(self, project_id: str) -> dict[str, Any]:
        directory = self.boba_error_doctor_review_path(project_id).parent.resolve()
        project_directory = self._project_dir(project_id).resolve()
        if directory.parent != project_directory or directory.name != "error_doctor_review":
            raise ValidationError("Invalid BOBA error doctor review reset path.")
        with self._lock:
            index_removed = self.boba_error_doctor_review_path(project_id).exists()
            self.boba_error_doctor_review_path(project_id).unlink(missing_ok=True)
    ...
    def boba_candidate_review_path(self, project_id: str) -> Path:
        return self._path(project_id, "candidate_review/index.json")

    def boba_candidate_review_registry_path(self, project_id: str, registry_id: str) -> Path:
        self._validate_candidate_review_record_id(registry_id, label="registry id")
        return self._path(project_id, f"candidate_review/registries/{registry_id}.json")

    def boba_candidate_review_session_path(self, project_id: str, session_id: str) -> Path:
        self._validate_candidate_review_record_id(session_id, label="session id")
        return self._path(project_id, f"candidate_review/sessions/{session_id}.json")

    def boba_candidate_review_snapshot_path(self, project_id: str, snapshot_id: str) -> Path:
        self._validate_candidate_review_record_id(snapshot_id, label="snapshot id")
        return self._path(project_id, f"candidate_review/snapshots/{snapshot_id}.json")

    def boba_candidate_review_action_path(self, project_id: str, action_id: str) -> Path:
        self._validate_candidate_review_record_id(action_id, label="action id")
        return self._path(project_id, f"candidate_review/actions/{action_id}.json")

    def boba_candidate_review_receipt_path(self, project_id: str, receipt_id: str) -> Path:
        self._validate_candidate_review_record_id(receipt_id, label="receipt id")
        return self._path(project_id, f"candidate_review/receipts/{receipt_id}.json")

    def boba_candidate_review_event_cursor_path(
        self, project_id: str, session_id: str
    ) -> Path:
        self._validate_candidate_review_record_id(session_id, label="session id")
        return self._path(project_id, f"candidate_review/event_cursors/{session_id}.json")

    def _write_boba_candidate_review_record(
        self, path: Path, payload: dict[str, Any], *, scope: str
    ) -> None:
        safe = sanitize_memory_payload(
            dict(payload),
            max_excerpt_chars=max(self.max_excerpt_chars, 1_200),
            path=f"boba.candidate_review.{scope}",
        )
        self._atomic_write_compact(path, safe)

    def _write_immutable_boba_candidate_review_record(
        self, path: Path, payload: dict[str, Any], *, scope: str, label: str
    ) -> None:
        safe = sanitize_memory_payload(
            dict(payload),
        ...
        with self._lock:
            if path.exists():
                ...
                if existing != safe:
                    raise ValidationError(
                        f"BOBA candidate review {label} are immutable."
                    )
                return
            self._atomic_write_compact(path, safe)

    def save_boba_candidate_review(self, project_id: str, payload: dict[str, Any]) -> None:
        self._write_boba_candidate_review_record(
            self.boba_candidate_review_path(project_id), payload, scope="index"
        )

    def load_boba_candidate_review(self, project_id: str) -> dict[str, Any] | None:
        raw = self._read(self.boba_candidate_review_path(project_id), None)
        return raw if isinstance(raw, dict) else None

    def save_boba_candidate_review_registry(
        self, project_id: str, registry_id: str, payload: dict[str, Any]
    ) -> None:
        self._write_immutable_boba_candidate_review_record(
            self.boba_candidate_review_registry_path(project_id, registry_id),
            payload,
            scope="registry",
            label="registry snapshots",
        )

    def load_boba_candidate_review_registry(
        self, project_id: str, registry_id: str
    ) -> dict[str, Any] | None:
        raw = self._read(
            self.boba_candidate_review_registry_path(project_id, registry_id), None
        )
        return raw if isinstance(raw, dict) else None

    def save_boba_candidate_review_session(
        self, project_id: str, session_id: str, payload: dict[str, Any]
    ) -> None:
        self._write_boba_candidate_review_record(
            self.boba_candidate_review_session_path(project_id, session_id),
            payload,
            scope="session",
        )

    def load_boba_candidate_review_session(
        self, project_id: str, session_id: str
    ) -> dict[str, Any] | None:
        raw = self._read(
            self.boba_candidate_review_session_path(project_id, session_id), None
        )
        return raw if isinstance(raw, dict) else None

    def delete_boba_candidate_review_session(self, project_id: str, session_id: str) -> bool:
        path = self.boba_candidate_review_session_path(project_id, session_id)
        with self._lock:
            existed = path.exists()
            path.unlink(missing_ok=True)
            self.boba_candidate_review_event_cursor_path(
                project_id, session_id
            ).unlink(missing_ok=True)
        return existed

    def save_boba_candidate_review_snapshot(
        self, project_id: str, snapshot_id: str, payload: dict[str, Any]
    ) -> None:
        self._write_boba_candidate_review_record(
            self.boba_candidate_review_snapshot_path(project_id, snapshot_id),
            payload,
            scope="snapshot",
        )

    def load_boba_candidate_review_snapshot(
        self, project_id: str, snapshot_id: str
    ) -> dict[str, Any] | None:
        raw = self._read(
            self.boba_candidate_review_snapshot_path(project_id, snapshot_id), None
        )
        return raw if isinstance(raw, dict) else None

    def save_boba_candidate_review_action(
        self, project_id: str, action_id: str, payload: dict[str, Any]
    ) -> None:
        self._write_immutable_boba_candidate_review_record(
            self.boba_candidate_review_action_path(project_id, action_id),
            payload,
            scope="action",
            label="action requests",
        )

    def load_boba_candidate_review_action(
        self, project_id: str, action_id: str
    ) -> dict[str, Any] | None:
        raw = self._read(self.boba_candidate_review_action_path(project_id, action_id), None)
        return raw if isinstance(raw, dict) else None

    def save_boba_candidate_review_receipt(
        self, project_id: str, receipt_id: str, payload: dict[str, Any]
    ) -> None:
        self._write_immutable_boba_candidate_review_record(
            self.boba_candidate_review_receipt_path(project_id, receipt_id),
            payload,
            scope="receipt",
            label="receipts",
        )

    def load_boba_candidate_review_receipt_for_action(
        self, project_id: str, action_id: str
    ) -> dict[str, Any] | None:
        self._validate_candidate_review_record_id(action_id, label="action id")
        directory = self._path(project_id, "candidate_review/receipts")
        if not directory.exists():
            return None
        for path in sorted(directory.glob("*.json"))[:256]:
            raw = self._read(path, None)
            if isinstance(raw, dict) and raw.get("candidate_action_request_id") == action_id:
                return raw
        return None

    def save_boba_candidate_review_event_cursor(
        self, project_id: str, session_id: str, payload: dict[str, Any]
    ) -> None:
        self._write_boba_candidate_review_record(
            self.boba_candidate_review_event_cursor_path(project_id, session_id),
            payload,
            scope="event_cursor",
        )

    def reset_boba_candidate_review_metadata(self, project_id: str) -> dict[str, Any]:
        directory = self.boba_candidate_review_path(project_id).parent.resolve()
        project_directory = self._project_dir(project_id).resolve()
        if directory.parent != project_directory or directory.name != "candidate_review":
            raise ValidationError("Invalid BOBA candidate review reset path.")
        with self._lock:
            index_removed = self.boba_candidate_review_path(project_id).exists()
            self.boba_candidate_review_path(project_id).unlink(missing_ok=True)
            sessions_directory = directory / "sessions"
            cursors_directory = directory / "event_cursors"
            snapshots_directory = directory / "snapshots"
            removed_sessions = (
                len(list(sessions_directory.glob("*.json")))
                if sessions_directory.exists()
                else 0
            )
            for child in (sessions_directory, cursors_directory, snapshots_directory):
                if child.exists():
                    shutil.rmtree(child)
        return {
            ...
            "editorial_history_preserved": True,
            "explanation_records_preserved": True,
            "creative_records_preserved": True,
            "review_ui_history_preserved": True,
            "workflow_history_preserved": True,
            "artifact_history_preserved": True,
            "validation_history_preserved": True,
    ...
    @staticmethod
    def _validate_repair_plan_review_record_id(value: str, *, label: str) -> str:
        if not re.fullmatch(r"[A-Za-z0-9_.:-]{1,180}", value):
            raise ValidationError(f"Invalid BOBA repair plan review {label}.")
        return value

    def boba_repair_plan_review_path(self, project_id: str) -> Path:
        return self._path(project_id, "repair_plan_review/index.json")

    def boba_repair_plan_review_registry_path(
        self, project_id: str, record_id: str
    ) -> Path:
        self._validate_repair_plan_review_record_id(record_id, label="registry id")
        return self._path(project_id, f"repair_plan_review/registries/{record_id}.json")

    def boba_repair_plan_review_session_path(self, project_id: str, record_id: str) -> Path:
        self._validate_repair_plan_review_record_id(record_id, label="session id")
        return self._path(project_id, f"repair_plan_review/sessions/{record_id}.json")

    def boba_repair_plan_review_snapshot_path(self, project_id: str, record_id: str) -> Path:
        self._validate_repair_plan_review_record_id(record_id, label="snapshot id")
        return self._path(project_id, f"repair_plan_review/snapshots/{record_id}.json")

    def boba_repair_plan_review_action_path(self, project_id: str, record_id: str) -> Path:
        self._validate_repair_plan_review_record_id(record_id, label="action id")
        return self._path(project_id, f"repair_plan_review/actions/{record_id}.json")

    def boba_repair_plan_review_receipt_path(self, project_id: str, record_id: str) -> Path:
        self._validate_repair_plan_review_record_id(record_id, label="receipt id")
        return self._path(project_id, f"repair_plan_review/receipts/{record_id}.json")

    def boba_repair_plan_review_event_cursor_path(
        self, project_id: str, session_id: str
    ) -> Path:
        self._validate_repair_plan_review_record_id(session_id, label="session id")
        return self._path(
            project_id, f"repair_plan_review/event_cursors/{session_id}.json"
        )

    def _write_boba_repair_plan_review_record(
        self, path: Path, payload: dict[str, Any], *, scope: str
    ) -> None:
        safe = sanitize_memory_payload(
            dict(payload),
            max_excerpt_chars=max(self.max_excerpt_chars, 1_200),
            path=f"boba.repair_plan_review.{scope}",
        )
        self._atomic_write_compact(path, safe)

    def _write_immutable_boba_repair_plan_review_record(
        self, path: Path, payload: dict[str, Any], *, scope: str, label: str
    ) -> None:
        safe = sanitize_memory_payload(
            dict(payload),
        ...
        with self._lock:
            if path.exists():
                ...
                if existing != safe:
                    raise ValidationError(
                        f"BOBA repair plan review {label} are immutable."
                    )
                return
            self._atomic_write_compact(path, safe)

    def save_boba_repair_plan_review(
        self, project_id: str, payload: dict[str, Any]
    ) -> None:
        self._write_boba_repair_plan_review_record(
            self.boba_repair_plan_review_path(project_id), payload, scope="index"
        )

    def load_boba_repair_plan_review(self, project_id: str) -> dict[str, Any] | None:
        raw = self._read(self.boba_repair_plan_review_path(project_id), None)
        return raw if isinstance(raw, dict) else None

    def save_boba_repair_plan_review_registry(
        self, project_id: str, record_id: str, payload: dict[str, Any]
    ) -> None:
        self._write_immutable_boba_repair_plan_review_record(
            self.boba_repair_plan_review_registry_path(project_id, record_id),
            payload,
            scope="registry",
            label="registry snapshots",
        )

    def load_boba_repair_plan_review_registry(
        self, project_id: str, record_id: str
    ) -> dict[str, Any] | None:
        raw = self._read(
            self.boba_repair_plan_review_registry_path(project_id, record_id), None
        )
        return raw if isinstance(raw, dict) else None

    def save_boba_repair_plan_review_session(
        self, project_id: str, record_id: str, payload: dict[str, Any]
    ) -> None:
        self._write_boba_repair_plan_review_record(
            self.boba_repair_plan_review_session_path(project_id, record_id),
            payload,
            scope="session",
        )

    def load_boba_repair_plan_review_session(
        self, project_id: str, record_id: str
    ) -> dict[str, Any] | None:
        raw = self._read(
            self.boba_repair_plan_review_session_path(project_id, record_id), None
        )
        return raw if isinstance(raw, dict) else None

    def save_boba_repair_plan_review_snapshot(
        self, project_id: str, record_id: str, payload: dict[str, Any]
    ) -> None:
        self._write_boba_repair_plan_review_record(
            self.boba_repair_plan_review_snapshot_path(project_id, record_id),
            payload,
            scope="snapshot",
        )

    def load_boba_repair_plan_review_snapshot(
        self, project_id: str, record_id: str
    ) -> dict[str, Any] | None:
        raw = self._read(
            self.boba_repair_plan_review_snapshot_path(project_id, record_id), None
        )
        return raw if isinstance(raw, dict) else None

    def save_boba_repair_plan_review_action(
        self, project_id: str, record_id: str, payload: dict[str, Any]
    ) -> None:
        self._write_immutable_boba_repair_plan_review_record(
            self.boba_repair_plan_review_action_path(project_id, record_id),
            payload,
            scope="action",
            label="action requests",
        )

    def load_boba_repair_plan_review_action(
        self, project_id: str, record_id: str
    ) -> dict[str, Any] | None:
        raw = self._read(
            self.boba_repair_plan_review_action_path(project_id, record_id), None
        )
        return raw if isinstance(raw, dict) else None

    def save_boba_repair_plan_review_receipt(
        self, project_id: str, record_id: str, payload: dict[str, Any]
    ) -> None:
        self._write_immutable_boba_repair_plan_review_record(
            self.boba_repair_plan_review_receipt_path(project_id, record_id),
            payload,
            scope="receipt",
            label="receipts",
        )

    def load_boba_repair_plan_review_receipt(
        self, project_id: str, record_id: str
    ) -> dict[str, Any] | None:
        raw = self._read(
            self.boba_repair_plan_review_receipt_path(project_id, record_id), None
        )
        return raw if isinstance(raw, dict) else None

    def load_boba_repair_plan_review_receipt_for_action(
        self, project_id: str, action_id: str
    ) -> dict[str, Any] | None:
        self._validate_repair_plan_review_record_id(action_id, label="action id")
        directory = self._path(project_id, "repair_plan_review/receipts")
        if not directory.exists():
            return None
        for path in sorted(directory.glob("*.json"))[:256]:
            raw = self._read(path, None)
            if isinstance(raw, dict) and (
                raw.get("repair_plan_action_request_id") == action_id
            ):
                return raw
        return None

    def delete_boba_repair_plan_review_session(
        self, project_id: str, session_id: str
    ) -> bool:
        path = self.boba_repair_plan_review_session_path(project_id, session_id)
        with self._lock:
            existed = path.exists()
            path.unlink(missing_ok=True)
            self.boba_repair_plan_review_event_cursor_path(
                project_id, session_id
            ).unlink(missing_ok=True)
        return existed

    def save_boba_repair_plan_review_event_cursor(
        self, project_id: str, session_id: str, payload: dict[str, Any]
    ) -> None:
        self._write_boba_repair_plan_review_record(
            self.boba_repair_plan_review_event_cursor_path(project_id, session_id),
            payload,
            scope="event_cursor",
        )

    def reset_boba_repair_plan_review_metadata(self, project_id: str) -> dict[str, Any]:
        """Remove only panel UI metadata. Every owner history is preserved."""
        directory = self.boba_repair_plan_review_path(project_id).parent.resolve()
        project_directory = self._project_dir(project_id).resolve()
        if directory.parent != project_directory or directory.name != "repair_plan_review":
            raise ValidationError("Invalid BOBA repair plan review reset path.")
        with self._lock:
            index_removed = self.boba_repair_plan_review_path(project_id).exists()
            self.boba_repair_plan_review_path(project_id).unlink(missing_ok=True)
            sessions_directory = directory / "sessions"
            removed_sessions = (
                len(list(sessions_directory.glob("*.json")))
                if sessions_directory.exists()
        ...
        return {
            ...
            "quality_preservation_records_preserved": True,
            "execution_handoff_records_preserved": True,
            "rejected_strategy_records_preserved": True,
            "root_cause_records_preserved": True,
            "incident_records_preserved": True,
            "recovery_history_preserved": True,
            ...
            "final_decision_history_preserved": True,
            "review_ui_history_preserved": True,
            "error_doctor_review_history_preserved": True,
            "source_media_removed": False,
            "accepted_outputs_removed": False,
            "code_modified": False,
            "artifacts_modified": False,
